document.write('\
\
<h2 id="srvpgm_organisation">*SRVPGM Organisation</h2>\
<p>\
The service programs all have the same organisation of source members. There is always one source \
member named AAAA_BBBB that contains the binder source, where AAAA is the first part of the service \
program name (CSYS or CAPP) and BBBB is the second part of the name. For example for service program \
CSYSBASE (source file CSYSBASES) the binder source is in source member CSYS_BASE.\
</p>\
<p>\
For every object that is part of the service program there is one source member having the same name as \
...
of the service programs such as files and programs. When there are files or data areas then there is probably \
...
did not already exist (and were created by B1) are loaded.\
</p>\
<p>\
...
that it only contains definitions that are used by the service program itself, not necessarily only used by \
one specific module. Private include members should never be included by consumers of the service program.\
</p>\
<p>\
...
INCL_DEFS member. Should always be included by INCL_XX members of the service program.\
</span>\
<span class="g21"><span class="bullet1">&#9679;</span>&nbsp;INCL_XX</span>\
<span class="g22">\
Contains public definitions, mostly prototypes, that are specific for module XX. To be included by \
consumers of the service program that only need functionality provided by that module. Is also always \
included by the module itself.\
</span>\
...
necessary) and all INCL_XX members. Included by consumers of the service program that are not modules of \
...
<span class="g61"><span class="bullet1">&#9679;</span>&nbsp;INCL__XX</span>\
<span class="g62">\
Contains private definitions, mostly prototypes, that are specific for module XX. To be included by modules \
...
service program. If the attribute types are public then member INCL_DEFS should define an external data structure \
based on this file, which name should be <code>ErfXXXXXXXXFR</code>. If the attribute types are private and should only be \
visible within the service program then the "Erf" external data structure should be defined in the INCL__MODD member. \
...
the service program\'s modules. Their prototypes are defined in the INCL_XX member of the module. Internal \
procedures are only used by modules of the service program itself. Their names are prefixed with "I_". Their \
prototypes are defined in the INCL__XX member of the module. Unlike public and internal procedures, local procedures \
...
identifies the category. For example DT for date/time functions, TU for text utilities, etc. Most service programs also \
have an XX (miscellaneous) module (not to be confused with XX to mean a generic id). Module XX of a service \
program contains the procedures that are not (yet) put into a specific category. Applications should not \
include INCL_XX as these procedures may eventually be moved to a specific category. Use INCL_API instead.\
</p>\
<p>\
Every service program has a RM module (Resource Management). It must always provide public procedure <code>RlsSpXXXXXXXX</code> \
which releases all resources managed by the service program. For example to close all open files or to release allocated \
memory. It does this by calling procedure <code>I_RlsModXX</code> for every module in the service program. Thus, every module \
must implement this internal procedure to release all resources that are managed by that module (even if there are \
no resources to release). When an application ends it should call <code>RlsSpXXXXXXXX</code> (e.g. <code>RlsSpCSYSBASE</code>) \
...
The complete ordering is: CSYSBASE&ltCSYSSQLS&ltCAPPBASE&ltCAPPSSAM&ltCAPPDISP.\
</p>\
\
');
